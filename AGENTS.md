...
- `KJ 2.0 环境镜像模板版`
  ...
  - 镜像：`ghcr.io/myjr2015/codex-wan22-kj-comfy:cuda129-py312-kj-v1`
  - Dockerfile：`docker/wan22-kj-comfy-env/Dockerfile`
  - Vast template helper：`scripts/create_vast_wan22_kj_env_template.ps1`
  - 状态：实验可跑；默认走 GHCR 构建，保留 DockerHub 可选路径，只预装 KJ custom nodes 和 Python 依赖，不包含模型权重，不使用 Vast volume。
- `KJ 2.0 背景/Mask失败版`
