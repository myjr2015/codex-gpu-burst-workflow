...
if ([string]::IsNullOrWhiteSpace($R2AccountId) -or [string]::IsNullOrWhiteSpace($R2AccessKeyId) -or [string]::IsNullOrWhiteSpace($R2SecretAccessKey)) {
    throw "R2 credentials missing. Provide R2 credentials, or set matching environment variables/api.txt."
}
...
& D:\code\YuYan\python\python.exe $uploadScript `
    ...
    --remote-prefix "$($R2Prefix.TrimEnd('/'))/$JobName/output" `
    --public-base-url $R2PublicBaseUrl

if ($LASTEXITCODE -ne 0) {
    throw "Failed to upload KJ segmented result to R2."
}

$resultName = [System.IO.Path]::GetFileName($resolvedResult)
$remoteKey = "$($R2Prefix.TrimEnd('/'))/$JobName/output/$resultName"
...
Write-Host "uploaded result to $($R2Prefix.TrimEnd('/'))/$JobName/output"
