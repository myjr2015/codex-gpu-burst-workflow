...
- Turkey is only a speed-risk signal now; do not hard-exclude it unless the current run explicitly asks for risky-region exclusion
...
4. record `driver_version` for diagnostics, but do not reject a machine only because the major version is below `580`
...
Choose the cheapest non-`CN` offer that satisfies the runtime assumptions; treat `TR` by actual speed and bootstrap logs.
...
- use a 24GB class NVIDIA card; record driver version but judge compatibility from CUDA / torch / bootstrap logs
